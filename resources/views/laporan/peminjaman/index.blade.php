<x-layouts.dashboard title="Laporan Peminjaman">
    ...
    <div class="space-y-5">
        ...
        <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <h2 class="mt-1 text-2xl font-semibold text-slate-950">Laporan Peminjaman</h2>
                    <p class="mt-1 text-sm text-slate-500">Data buku yang sedang aktif dipinjam (status: Dipinjam & Terlambat).</p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <a href="{{ route('laporan.peminjaman.pdf') }}"
        ...
        {{-- FILTER --}}
        <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
            <form method="GET" action="{{ route('laporan.peminjaman.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
                <div class="flex-1">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($data as $item)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-5 py-4">
                                    @php $jatuhTempo = \Carbon\Carbon::parse($item->tgl_jatuh_tempo); @endphp
                                    <span class="{{ $jatuhTempo->isPast() ? 'text-red-600 font-semibold' : 'text-slate-600' }}">
                                        {{ $jatuhTempo->format('d/m/Y') }}
                                    </span>
                                </td>
                                <td class="px-5 py-4 text-center">
                                    @if ($item->status === 'dipinjam')
                                        <span class="inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">Dipinjam</span>
                                    @else
                                        <span class="inline-flex rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700">Terlambat</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="px-5 py-10 text-center text-slate-500">
                                    Tidak ada data peminjaman aktif yang ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
