<x-layouts.dashboard title="Laporan Denda">
    ...
    <div class="space-y-5">
        ...
        <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <h2 class="mt-1 text-2xl font-semibold text-slate-950">Laporan Denda</h2>
                    <p class="mt-1 text-sm text-slate-500">Data seluruh transaksi yang memiliki nominal denda keterlambatan.</p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <a href="{{ route('laporan.denda.excel') }}"
                    ...
                    <a href="{{ route('laporan.denda.pdf') }}"
        ...
        {{-- FILTER --}}
        <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
            <form method="GET" action="{{ route('laporan.denda.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
                <div class="flex-1">
                ...
                <div class="w-full sm:w-44">
                    <label class="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wide">Status Denda</label>
                    <select name="status_denda" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none">
                        <option value="">Semua Status</option>
                        <option value="belum_lunas" {{ ($status_denda ?? '') === 'belum_lunas' ? 'selected' : '' }}>Belum Lunas</option>
                        <option value="lunas" {{ ($status_denda ?? '') === 'lunas' ? 'selected' : '' }}>Lunas</option>
                    </select>
                </div>
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('laporan.denda.index') }}"
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                        <tr>
                            ...
                            <th class="px-5 py-3">Tgl. Jatuh Tempo</th>
